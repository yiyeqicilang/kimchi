...
class ZH4 extends Base
{
    ...
    {
        if( !preg_match(sprintf('/^(([0-9]\%s){0,9}[0-9])\%s(([0-9]\%s){0,9}[0-9])\%s(([0-9]\%s){0,9}[0-9])\%s(([0-9]\%s){0,9}[0-9])$/',$this->codeSep,$this->lineSep,$this->codeSep,$this->lineSep,$this->codeSep,$this->lineSep,$this->codeSep),$sCodes) ) return false;
    ...
    public function assertLevel($levelId, $sCodes, Array $numbers)
    {
        ...
        if ($levelId == '1') {
            $preg = "|[" . str_replace($this->codeSep, '', $aCodes[0]) . "][" . str_replace($this->codeSep, '', $aCodes[1]) . "][" . str_replace($this->codeSep, '', $aCodes[2]) . "][" . str_replace($this->codeSep, '', $aCodes[3]) . "]|";
            if (preg_match($preg, implode("", $numbers))) {
                return 1;
            }
        } elseif ($levelId == '2') {
            $preg = "|[" . str_replace($this->codeSep, '', $aCodes[1]) . "][" . str_replace($this->codeSep, '', $aCodes[2]) . "][" . str_replace($this->codeSep, '', $aCodes[3]) . "]|";
            if (preg_match($preg, implode("", $numbers))) {
                $times = count(explode($this->codeSep,$aCodes[0]));
                return $times;
            }
        } elseif ($levelId == '3') {
            $preg = "|[" . str_replace($this->codeSep, '', $aCodes[2]) . "][" . str_replace($this->codeSep, '', $aCodes[3]) . "]|";
            if (preg_match($preg, implode("", $numbers))) {
